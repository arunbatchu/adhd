...
**Distractibility** is attention's capture problem from Chapter 1 running at full strength: external events — a noise, a notification, a person walking past — yank the spotlight off task, and each yank costs the re-entry toll of finding your place again. The tax is bigger than the interruptions themselves: twenty small captures can hollow out a work hour while looking, from the outside, like an hour of work. Watch it happen to Omar, doing homework at the kitchen table: the dishwasher clicks (capture, 5 seconds), he re-finds his place in the math problem (re-entry, 40 seconds), his sister opens the fridge (capture), re-entry, a phone buzzes in another room — not even his phone (capture), re-entry. In forty minutes Omar has done maybe twelve minutes of math, is genuinely tired, and has "been doing homework the whole time" — every word of which is true. His father, seeing the half-empty worksheet, concludes he goofed off. Both are reporting honestly from different sides of the same hour. The re-entry toll is the invisible part: each capture costs a few seconds, but each *return* costs the working-memory rebuild of where-was-I, and those rebuilds are what consume the hour. This is why the fixes in Chapter 9 target the captures (headphones, phone in another room, cleared table) rather than demanding better recovery.

**Mind wandering** is the internal version — the default mode network from Chapter 2 flickering on mid-task, so you surface three paragraphs later having read every word and absorbed none. External noise-proofing doesn't touch this one, which is why "just work somewhere quiet" only half-helps.
...
**Forgetfulness** in ADHD is Chapter 2's memory chapter happening daily: appointments, commitments, the reason you walked into this room — overwhelmingly failures of prospective memory and working-memory encoding, not the filing cabinet. Its signature is unevenness (2014's insult: perfectly preserved; this morning's promise: gone), which is exactly what makes it read as selective caring. It isn't selective by importance; it's selective by *cue* — what got externalized, what fired at the right moment. Run one week of Rosa's forgetting as a worked example: Monday she forgets the dentist (made three weeks ago, no reminder set — time-based prospective memory, the least reliable kind). Wednesday she remembers her nephew's birthday the moment she sees a cake in a shop window (event-based, cued — fired perfectly). Friday she forgets the thing her partner told her while she was mid-email (never encoded; working memory was full). Sort the week by "does Rosa care?" and it's random noise — she loves the nephew and the partner equally, and the dentist matters most. Sort it by *cue availability* and every data point lands exactly where Chapter 2 predicts. That sorting exercise, done together over a real week, does more to end the "you'd remember if you cared" fight than any apology can.

**Losing things** is the same machinery applied to objects: keys, phone, wallet, glasses, the permission slip — set down mid-thought with attention already three steps ahead, so the location was never encoded at all. You can't retrieve a memory that was never filed. This is why retracing steps fails and why the fix is spatial rather than mnemonic: one bowl by the door, one pocket, one charger spot — locations doing the remembering. Chapter 9 calls this giving everything a home.
